# token_parallel 独立验证目录

KDA Kernel-2（Token Parallel：对角线 Aqk/Akk）的独立、可上传验证目录。
...
├── src/
│   ├── __init__.py
│   └── token_parallel_kernel.py   三种实现：CPU 参考 token_parallel_ref、
│                                   torch_npu 元算子 token_parallel_torch、
│                                   triton kernel token_parallel_triton
└── util/
    ├── __init__.py
    └── csvb64.py        base64 CSV 编解码（保证张量逐 bit 往返一致）
```

## 算子数学（Kernel-2：Token Parallel 对角线块）

每个 token 分配一个独立 CTA，在自己所属的 sub-chunk 内向前遍历历史 token，
计算 gated dot product（同 sub-chunk 内，`j <= i`）：
...
- `Aqk [B,T,H,BT]`：列索引 = j 在 chunk 内的位置（`j % BT`）；
- `Akk [B,T,H,BC]`：列索引 = j - i_ts（sub-chunk 内相对偏移），对角线元素为 0。
...
| 步骤 | 实现 | 用途 |
|------|------|------|
| 1 | `token_parallel_torch`（torch_npu 元算子图） | 精度/性能**基准** |
| 2 | `token_parallel_triton`（triton kernel） | 被测对象 |
| 3 | `token_parallel_ref`（纯 torch CPU，逐 token 循环） | ground truth（`--no-ref` 可跳过） |

每个 case 输出：`torch_npu 耗时 / triton 耗时 / speedup`，以及
`max|triton-torch|`（Aqk / Akk 分开）与 `max|torch-ref|`、`max|triton-ref|` 的精度对。
...
```bash
docker exec -w /docker/zhm/0505_skill_test/sonnet/sglang/token_parallel triton-ascend-env-zhm bash -c '
...
```bash
python3 -c "from src.token_parallel_kernel import token_parallel_ref; import torch
Aqk,Akk=token_parallel_ref(torch.randn(1,128,2,64),torch.randn(1,128,2,64),torch.randn(1,128,2,64),torch.randn(1,128,2),0.1768); print(Aqk.shape, Akk.shape)"
```
...
| 上游（sglang/kernels/ops/attention/fla/chunk_intra_token_parallel.py） | 本目录 |
|----------------------------------------------------------------------|--------|
| `chunk_kda_fwd_kernel_intra_token_parallel` | `src/token_parallel_kernel.py::_token_parallel_kernel` |
| `chunk_kda_fwd_intra_token_parallel` (python driver) | `src/token_parallel_kernel.py::token_parallel_triton` |
| CPU 参考（测试） | `src/token_parallel_kernel.py::token_parallel_ref` |
| 性能基准（新）torch_npu 算子图 | `src/token_parallel_kernel.py::token_parallel_torch` |
...
实测（910B2, triton-ascend, warmup=2/repeats=5, `--no-ref`）：15/15 PASS；msprof
内核时间口径下 torch_npu 拼接 vs triton 的**合计加速比 ≈ 24x**（torch 算子图含
Transpose/ViewCopy/Exp2/ReduceSum 等多条 kernel，triton 每 token 单 CTA 单 kernel）。