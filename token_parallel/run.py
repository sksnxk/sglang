...
"""KDA Kernel-2 (Token Parallel: 对角线 Aqk/Akk) 精度 & 性能对比测试。

    python3 run.py [CSV] [--repeats N] [--warmup N] [--max-diff LIMIT] [--no-ref]

流程（与本目录 README/DESIGN 一致）:
  1. 读 CSV 中每个用例 (q, k, gk, beta, scale);
  2. 先跑 **torch_npu 元算子** ``token_parallel_torch`` —— 精度 & 性能**基准**;
  3. 再跑 **triton kernel** ``token_parallel_triton`` —— 被测对象;
  4. 精度: 两个实现各自与 CPU 参考 ``token_parallel_ref`` 的 max-diff
...
from src.token_parallel_kernel import (  # noqa: E402
    _BT,
    _BC,
    token_parallel_ref,
    token_parallel_torch,
    token_parallel_triton,
)
...
def main(argv=None):
    p = argparse.ArgumentParser(description="token-parallel Aqk/Akk: torch_npu vs triton")
    ...
    for cid, tensors in cases.items():
        ...
        At, Bt = token_parallel_torch(q, k, gk, beta, scale)
        t_torch = _timeit(
            lambda: token_parallel_torch(q, k, gk, beta, scale),
            a.warmup, a.repeats, sync,
        )

        # ── 2) triton kernel ──
        Ap, Bp = token_parallel_triton(q, k, gk, beta, scale)
        t_tri = _timeit(
            lambda: token_parallel_triton(q, k, gk, beta, scale),
            a.warmup, a.repeats, sync,
        )

        # ── 3) 精度: 两个实现彼此对比, 及(可选)各自 vs CPU 参考 ──
        d_qk = _maxdiff(Ap, At)   # triton Aqk vs torch Aqk
        d_kk = _maxdiff(Bp, Bt)   # triton Akk vs torch Akk
        msg = f"tri_vs_torch Aqk={d_qk:.3e} Akk={d_kk:.3e}"
        d_pass = d_qk < a.max_diff and d_kk < a.max_diff
        if not a.no_ref:
            Ar, Br = token_parallel_ref(
                tensors["q"], tensors["k"], tensors["gk"], tensors["beta"], scale,
            )
