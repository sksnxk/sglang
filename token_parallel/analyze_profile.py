...
背景（见 token_parallel/DESIGN.md §6/§7）
-----------------------------------------------
* run.py 里有两个实现：
    - ``token_parallel_torch``：torch_npu 元算子拼接——按 sub-chunk 的 6 维 broadcast
      + ``exp2`` + ``mul`` + ``sum(-1)`` 归约，有多条 kernel（另有 zeros/tril/eye/
      permute/reshape 等）。这些 kernel 都算做 ``torch_npu 拼接时间``；
    - ``token_parallel_triton``（triton kernel，op 名 ``_token_parallel_kernel``）：
      单 kernel，每 token 一个 CTA。
...
TRITON_NAMES = ("_token_parallel_kernel",)
