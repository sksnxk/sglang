...
背景（见 recompute_w_u/DESIGN.md §6/§7）
-----------------------------------------------
* run.py 里有两个实现：
    - ``recompute_w_u_torch``：torch_npu 元算子拼接——按 chunk 批量化的 permute + matmul
      + reshape + exp2 + elementwise mul，有多条 kernel（另有 zeros/cat）。这些 kernel 都
      算做 ``torch_npu 拼接时间``；
    - ``recompute_w_u_triton``（triton kernel，op 名 ``_recompute_w_u_kernel``）：单 kernel。
...
TRITON_NAMES = ("_recompute_w_u_kernel",)
