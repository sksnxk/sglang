...
"""CSV 编码/解码工具（base64 紧凑格式，服务于 recompute_w_u 测试）。

为何用 base64 而不是直接写数值?
------------------------------
recompute_w_u 的输入是 fp32 随机张量（k / v / beta / A=Akk_inv / gk）与标量 scale，
...
def to_text(db: dict, title: str = "KDA recompute_w_u test cases") -> str:
    ...
    out = io.StringIO()
    out.write(f"# {title}\n")
    out.write("# 每个 case 内的张量名称: k / v / beta / A / gk / scale\n")
...
if __name__ == "__main__":
    db = {
        "t1": {
            ...
            "A": torch.randn(1, 128, 2, 64),
            "gk": torch.randn(1, 128, 2, 64),
