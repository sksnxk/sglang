...
参数:
    ...
    --case xx         只输出单个 case(如 tiny_default)
"""
import argparse
import csv
import os
...
# delta_rule_h_triton 每次调用在 op_summary 里出现一次, 故每个 case 的 triton kernel
# 调用次数 = 1(warmup) + repeats + 1. 脚本用 TRITON_PER_CASE 界定 case 边界。
TRITON_PER_CASE = 8

CASE_IDS = [
    "tiny_default", "tiny_partial_chunk", "tiny_single_head", "tiny_H3", "tiny_T65",
    "tiny_T96", "tiny_T1", "tiny_T2", "tiny_T100", "tiny_T127", "big_B2_H3",
    "big_B2_T193", "big_T256", "tiny_T255", "tiny_T2562",
]
TRITON_PREFIX = "_delta_rule_h_kernel"
