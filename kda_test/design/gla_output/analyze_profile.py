...
用法:
    ...
    python3 analyze_profile.py --latest-dir prof --scope k6  # 只统计 gla_output_torch 的算子

背景（见 gla_output/DESIGN.md §6/§7）
-----------------------------------------------
* run.py 里有两个实现：
    - ``gla_output_torch``：torch_npu 元算子拼接（exp2/mul/matmul/tril/mul/matmul/add
      有多条 kernel），另有对尾 chunk 的 ``zeros``/``cat``。
    - ``gla_output_kernel``（triton kernel，op 名 ``chunk_gla_fwd_kernel_o``）：单 kernel。
* msprof（--task-time 默认开）会把每个设备 kernel 记成 op_summary 一行，列里有
  ``Task Duration(us)``（AIV 侧时间，含 aicore/aiv）。
* 本脚本聚合每个 op name 的总耗时。默认 ``--scope=all`` 把所有非 triton kernel
  求和为 ``torch_npu 拼接总时间``（含上游 K1-K5 kernel）；``--scope=k6`` 只统计
  ``gla_output_torch`` 实际发出的 ``aclnn*`` 算子，与 run.py 的 wall-clock 口径
  一致。triton kernel 的加速比 = ``torch_us / tri_us``。
"""
...
TRITON_NAMES = ("chunk_gla_fwd_kernel_o",)

# gla_output_torch (torch_npu 算子图) 实际发出的算子名模式。当 --scope=k6
# 时只统计这些算子 + K6 triton kernel，与 run.py 的 wall-clock 口径一致，
# 排除上游 K1-K5 kernel 的时间。
K6_TORCH_PREFIXES = (
    "aclnnAdd_",          # o = o_cross + o_intra
    "aclnnCat_",          # 尾 chunk 补零拼接
    "aclnnExp2_",         # exp2(g)
    "aclnnInplaceZero_",  # zeros
    "aclnnInplaceOne_",   # ones (tril 的输入)
    "aclnnMatmul_",       # 跨块 / 块内 matmul
    "aclnnMul_",          # q*scale / qg*... / A*mask
    "aclnnMuls_",         # scale 标量广播
    "aclnnTril_",         # 下三角 mask
    "aclnnArange_",       # arange (tril 内部)
)
...
def main(argv=None):
    ...
    p.add_argument("--scope", choices=["all", "k6"], default="all",
                   help="all=把所有非 triton kernel 算作 torch_npu 拼接 (含上游 K1-K5); "
                        "k6=只统计 gla_output_torch 实际发出的算子 (aclnn* 系列)，"
                        "与 run.py 的 wall-clock 口径一致")
    a = p.parse_args(argv)
    ...
    if a.scope == "k6":
        # 只统计 gla_output_torch 发出的 aclnn* 算子 + K6 triton kernel
        is_torch = lambda name: (  # noqa: E731
            not is_tri(name)
            and any(name.startswith(p) for p in K6_TORCH_PREFIXES)
        )
    else:
        is_torch = lambda name: not is_tri(name)  # noqa: E731

    n_torch = sum(1 for name, *_ in rows if is_torch(name))
    torch_us = sum(d for n, d in agg.items() if is_torch(n)) + n_torch * pad
    tri_us = sum(d for n, d in agg.items() if is_tri(n))
    speedup = torch_us / tri_us if tri_us > 0 else float("nan")

    print(f"metric={a.metric}  scope={a.scope}  (torch_npu 每 kernel +{pad:.1f}us 启动开销)")
    ...
    for name in sorted(agg):
        if is_tri(name):
            ...
            tag = "skip" if a.scope == "k6" else "torch"
        if tag == "skip":
            continue
