...
# 的方式运行 gla_output (Kernel 6) 的精度&性能测试。
#
# 用法:
...
#
# 说明: 本测试驱动依赖 sglang 的上游 K1-K5 算子来产生 Kernel 6 的输入
#       (g_cumsum / v_new / Aqk / h), 因此 PYTHONPATH 需包含 sglang/python。
...
# SGLang 上游 K1-K5 在 python/sglang 下
...
#      msprof --output=./prof --application="python3 run.py [args]"      # (未显式给 --repeats/--warmup 时自动用 5/2)
