...
"""CSV 编码/解码工具（base64 紧凑格式，服务于 token-parallel 对角线 Aqk/Akk 测试）。

为何用 base64 而不是直接写数值?
------------------------------
token-parallel 的输入是 fp32 随机张量（q / k / gk / beta）与标量 scale，
...
与 gate_chunk_cumsum/util/csvb64.py 的区别: 去掉 gate 专用的 ``run_csv``
（其底部 import ``src.gate_kernel``），保留通用的编/解码函数，完全中立。
"""
...
def to_text(db: dict, title: str = "KDA token-parallel test cases") -> str:
    ...
    out = io.StringIO()
    out.write(f"# {title}\n")
    out.write("# 每个 case 内的张量名称: q / k / gk / beta / scale\n")
...
if __name__ == "__main__":
    db = {
        "t1": {
            "q": torch.randn(1, 64, 1, 32),
            "k": torch.randn(1, 64, 1, 32),
            "gk": torch.randn(1, 64, 1, 32),
            "beta": torch.randn(1, 64, 1),
