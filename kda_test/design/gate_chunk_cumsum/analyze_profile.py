...
背景（见 gate_chunk_cumsum/DESIGN.md §6/§7）
-----------------------------------------------
* run.py 里有两个实现：
    - ``gate_cumsum_torch``：torch_npu 元算子拼接（add→logaddexp softplus→exp→mul→
      cumsum→mul 有多条 kernel），另有对尾 chunk 的 ``zeros``/``cat``。这些 kernel
      都算做 ``torch_npu 拼接时间``；
    - ``gate_chunk_cumsum``（triton kernel，op 名 ``_gate_cumsum_kernel``）：单 kernel。
...
# 以 gate_cumsum_torch 实际发出的算子为准：zeros/Add/LogAddExp/Exp/
# Mul(S)/Neg 是激活拼接(Cat 已被 zeros 覆盖或由 Add 统一)……
# 简化方案：除 triton kernel 外，所有内核都算“torch_npu 拼接”。
TRITON_NAMES = ("_gate_cumsum_kernel",)
...
def main(argv=None):
    ...
    # torch_ms 在这里的单位是「秒」(后续 *1e3 → ms)。pad 每 kernel 是 us:
    #   总 us = sum_us + n_torch * pad  → 秒 = 总us / 1e6
