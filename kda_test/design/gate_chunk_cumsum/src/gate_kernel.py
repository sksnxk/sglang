...
因此在无 NPU 设备的环境里也可以正常 import、并运行 ``gate_cumsum_ref``；
``gate_chunk_cumsum`` 在检测不到 NPU 时会自动退化为参考实现（便于纯 CPU
...
def gate_cumsum_torch(
    ...
    供与 triton kernel 做加速比对比。比 ``gate_cumsum_ref`` 快:
    ...
    参数与 ``gate_chunk_cumsum`` 相同。返回 [B,T,H,K] fp32(NPU 或 CPU device
    与输入一致)。
    """
    assert x.dim() == 4, f"x must be 4D [B,T,H,K], got shape {tuple(x.shape)}"
