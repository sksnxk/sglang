...
背景（见 inter_solve/DESIGN.md §6/§7）
-----------------------------------------------
* run.py 里有两个实现：
    - ``inter_solve_torch``：torch_npu 元算子拼接——按 chunk 批量化的 einsum +
      matmul + 前向替换循环 + 链式合并，有多条 kernel（另有 zeros/tril/eye/
      permute/reshape 等）。这些 kernel 都算做 ``torch_npu 拼接时间``；
    - ``inter_solve_triton``（triton kernel，op 名 ``_inter_solve_kernel``）：
      单 kernel，每 chunk/head 一个 CTA。
...
TRITON_NAMES = ("_inter_solve_kernel",)
